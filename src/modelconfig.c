 ...
 *
 * Purpose:   Given a model <hmm> with core amino acid probabilities,
 *            the null1 model <bg>, a desired search <mode>, and an
 *            expected target sequence length <L>; configure the
 *            frameshift-aware search model in <gm_fs> with lod scores
 *            relative to the background frequencies in <bg>.
 *
 *            The number of codon lengths supported is determined by
 *            <gm_fs->codon_lengths> (3 or 5).  For 5-codon profiles,
 *            quasicodons of length 1, 2, 4, and 5 are scored; for
 *            3-codon profiles, only lengths 2, 3, and 4 are scored.
 *            All (quasi)codon emission scores are adjusted by the
 *            appropriate frameshift probability.
 *
 * Returns:   <eslOK> on success; the profile <gm_fs> now contains
 *            scores and is ready for searching target sequences.
 *
 * Throws:    <eslEMEM> on allocation error.
 */
...
p7_ProfileConfig_fs(const P7_HMM *hmm, const P7_BG *bg, const ESL_GENCODE *gcode, P7_FS_PROFILE *gm_fs, int L_amino, int mode)
{
  ...
  /* Match emission scores.
   * Initialize all codon rows to -inf, then fill amino acid log-odds
   * into the amino acid section of each row.  With the rsc[codon][k]
   * layout we initialize by row (codon index), not by column (k).
   */
  sc[hmm->abc->K]    = -eslINFINITY; /* gap character    */
  ...
  /* Assign scores, amino acids, and indel positions to all codons and
   * quasicodons.  The codon index macros differ between the 5-codon and
   * 3-codon systems, so the two paths are kept in separate branches.
   */
  if (gm_fs->codon_lengths == 5) {
    for (k = 1; k <= hmm->M; k++) {
      for (x = 0; x < 4; x++) {
        for (w = 0; w < 4; w++) {
          for (v = 0; v < 4; v++) {
            ...
            if (p7P_MSC_AMINO5(gm_fs, k, a) >= p7P_MSC_CODON(gm_fs, k, codon_idx)) {
              p7P_MSC_CODON(gm_fs, k, codon_idx) = p7P_MSC_AMINO5(gm_fs, k, a);
              p7P_AMINO(gm_fs, k, codon_idx) = a;
              p7P_INDEL(gm_fs, k, codon_idx) = p7P___X;
            }

			codon_idx = p7P_CODON1_FS5(v); //X__
            if (p7P_MSC_AMINO5(gm_fs, k, a) >= p7P_MSC_CODON(gm_fs, k, codon_idx)) {
              p7P_MSC_CODON(gm_fs, k, codon_idx) = p7P_MSC_AMINO5(gm_fs, k, a);
              p7P_AMINO(gm_fs, k, codon_idx) = a;
			...
			/* two-nucleotide quasicodons (_XX, X_X, XX_) */
            codon_idx = p7P_CODON2_FS5(w, x); //_XX
            if (p7P_MSC_AMINO5(gm_fs, k, a) >= p7P_MSC_CODON(gm_fs, k, codon_idx)) {
              p7P_MSC_CODON(gm_fs, k, codon_idx) = p7P_MSC_AMINO5(gm_fs, k, a);
              p7P_AMINO(gm_fs, k, codon_idx) = a;
            ...
            codon_idx = p7P_CODON2_FS5(v, x); //X_X
			if (p7P_MSC_AMINO5(gm_fs, k, a) >= p7P_MSC_CODON(gm_fs, k, codon_idx)) {
              p7P_MSC_CODON(gm_fs, k, codon_idx) = p7P_MSC_AMINO5(gm_fs, k, a);
              p7P_AMINO(gm_fs, k, codon_idx) = a;
			...
			codon_idx = p7P_CODON2_FS5(v, w); //XX_
            if (p7P_MSC_AMINO5(gm_fs, k, a) >= p7P_MSC_CODON(gm_fs, k, codon_idx)) {
              p7P_MSC_CODON(gm_fs, k, codon_idx) = p7P_MSC_AMINO5(gm_fs, k, a);
              p7P_AMINO(gm_fs, k, codon_idx) = a;
            ...
            if (a == hmm->abc->Kp-2) { /* stop codon: use best single-substitution */
              for (subn = 0; subn < 4; subn++) {
				suba = gcode->basic[16 * subn + 4 * w + x];
                if (p7P_MSC_AMINO5(gm_fs, k, suba) >= p7P_MSC_CODON(gm_fs, k, codon_idx)) { 
				  p7P_MSC_CODON(gm_fs, k, codon_idx) = p7P_MSC_AMINO5(gm_fs, k, suba);
                  p7P_AMINO(gm_fs, k, codon_idx) = suba;
            ...
            for (u = 0; u < 4; u++) {
			  ...
			  for (t = 0; t < 4; t++) {
                /* five-nucleotide quasicodons (XXxxX, XxxXX, xxXXX) */ 
                codon_idx = p7P_CODON5_FS5(t, u, v, w, x);

				codon = 16 * t + 4 * u + x; //XXxxX
				a = gcode->basic[codon];
                if (p7P_MSC_AMINO5(gm_fs, k, a) >= p7P_MSC_CODON(gm_fs, k, codon_idx)) {
                  p7P_MSC_CODON(gm_fs, k, codon_idx) = p7P_MSC_AMINO5(gm_fs, k, a);
                  p7P_AMINO(gm_fs, k, codon_idx) = a;
      ...
      for (x = 0; x < 4; x++) {
        for (w = 0; w < 4; w++) {
          for (v = 0; v < 4; v++) {
            ...
            /* two-nucleotide quasicodons (_XX, X_X, XX_) */
            codon_idx = p7P_CODON2_FS3(w, x); //_XX
            if (p7P_MSC_AMINO3(gm_fs, k, a) > p7P_MSC_CODON(gm_fs, k, codon_idx)) {
              p7P_MSC_CODON(gm_fs, k, codon_idx) = p7P_MSC_AMINO3(gm_fs, k, a);
              p7P_AMINO(gm_fs, k, codon_idx) = a;
              p7P_INDEL(gm_fs, k, codon_idx) = p7P__XX;
            }

            codon_idx = p7P_CODON2_FS3(v, x); //X_X
            if (p7P_MSC_AMINO3(gm_fs, k, a) > p7P_MSC_CODON(gm_fs, k, codon_idx)) {
              p7P_MSC_CODON(gm_fs, k, codon_idx) = p7P_MSC_AMINO3(gm_fs, k, a);
              p7P_AMINO(gm_fs, k, codon_idx) = a;
              p7P_INDEL(gm_fs, k, codon_idx) = p7P_X_X;
            ...
            codon_idx = p7P_CODON2_FS3(v, w); //XX_
            if (p7P_MSC_AMINO3(gm_fs, k, a) > p7P_MSC_CODON(gm_fs, k, codon_idx)) {
              p7P_MSC_CODON(gm_fs, k, codon_idx) = p7P_MSC_AMINO3(gm_fs, k, a);
              p7P_AMINO(gm_fs, k, codon_idx) = a;
              p7P_INDEL(gm_fs, k, codon_idx) = p7P_XX_;
            ...
            if (a == hmm->abc->Kp-2) { /* stop codon: use best single-substitution */
              for (subn = 0; subn < 4; subn++) {
                suba = gcode->basic[16 * subn + 4 * w + x];
                if (p7P_MSC_AMINO3(gm_fs, k, suba) > p7P_MSC_CODON(gm_fs, k, codon_idx)) {
                  p7P_MSC_CODON(gm_fs, k, codon_idx) = p7P_MSC_AMINO3(gm_fs, k, suba);
                  p7P_AMINO(gm_fs, k, codon_idx) = suba;
                  p7P_INDEL(gm_fs, k, codon_idx) = p7P_xXX;
                }
                suba = gcode->basic[16 * v + 4 * subn + x];
...
utest_Config_fs(P7_HMM *hmm, P7_BG *bg, ESL_GENCODE *gcode)
{
  ...
  if ((gm_fs5 = p7_profile_fs_Create(hmm->M, hmm->abc, p7P_5CODONS)) == NULL)   esl_fatal(msg);
  if (p7_ProfileConfig_fs(hmm, bg, gcode, gm_fs5, 350, p7_LOCAL)     != eslOK)  esl_fatal(msg);
  if (p7_profile_fs_Validate(gm_fs5, NULL, 0.0001)                   != eslOK)  esl_fatal(msg);
