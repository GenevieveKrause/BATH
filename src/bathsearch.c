 ...
 */
 ...
 if ( cfg->qfmt == eslSQFILE_UNKNOWN ) {
  ...
  while (qhstatus == eslOK) 
  {
    ...
    p7_ProfileConfig_fs_New(hmm, info->bg, gcode, gm_fs5, 100, p7_LOCAL);  /* build framshift aware codon HMM */
    p7_ProfileConfig_fs_New(hmm, info->bg, gcode, gm_fs3, 100, p7_LOCAL);
    ...
    if (esl_opt_IsUsed(go, "--splice") && tophits_accumulator->N) { 
      ...
      p7_ProfileConfig_fs_New(hmm, info->bg, gcode, gm_tr, 100, p7_UNILOCAL); 
...
serial_loop(WORKER_INFO *info, ID_LENGTH_LIST *id_length_list, ESL_SQFILE *dbfp, char *firstseq_key, int n_targetseqs)
{
  ...
  while (sstatus == eslOK && (n_targetseqs==-1 || seq_id < n_targetseqs) ) 
  {
    ...
    if (info->pli->strands != p7_STRAND_BOTTOMONLY) 
    {
      ...
      p7_Pipeline_BATH(info->pli, info->om, info->om_fs, info->om_fs3, info->om_fs5, info->gm_fs5, info->scoredata, info->bg, info->th, info->pli->nseqs, dbsq_dna, info->wrk->orf_block, info->gcode, info->hw, p7_NOCOMPLEMENT);
    ...
    {   
      ...
      p7_Pipeline_BATH(info->pli, info->om, info->om_fs, info->om_fs3, info->om_fs5, info->gm_fs5, info->scoredata, info->bg, info->th, info->pli->nseqs, dbsq_dna, info->wrk->orf_block, info->gcode, info->hw, p7_COMPLEMENT); 
      p7_pipeline_Reuse_BATH(info->pli); // prepare for next search
  ...
  while (block->count > 0)
  {
    ...
    for (i = 0; i < block->count; ++i)
    {
      ...
      if (info->pli->strands != p7_STRAND_BOTTOMONLY) {
        ...
        p7_Pipeline_BATH(info->pli, info->om, info->om_fs, info->om_fs3, info->om_fs5, info->gm_fs5, info->scoredata, info->bg, info->th, block->first_seqidx + i, dnaSeq, info->wrk->orf_block, info->gcode, info->hw, p7_NOCOMPLEMENT);
        p7_pipeline_Reuse_BATH(info->pli); // prepare for next search
      ...
      if (info->pli->strands != p7_STRAND_TOPONLY) {
        ...
        p7_Pipeline_BATH(info->pli, info->om, info->om_fs, info->om_fs3, info->om_fs5, info->gm_fs5, info->scoredata, info->bg, info->th, block->first_seqidx + i, dnaSeq, info->wrk->orf_block, info->gcode, info->hw, p7_COMPLEMENT);
        p7_pipeline_Reuse_BATH(info->pli); // prepare for next search
